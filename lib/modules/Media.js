...
class Media {
     ...
     * @param isBackground
     */
    constructor(path, startTime, hasVideo, hasAudio, isBackground) {
        this.user = null;
        this.id = -1;
        this.duration = -1;
        ...
        this.path = path;
        if (!(hasAudio || hasVideo || isBackground))
            throw new Error('media must contain audio, video or a background image.');
        if (isBackground && (hasAudio || hasVideo))
            throw new Error('Media cannot have background combined with audio or video');
    ...
    init() {
        ...
        return new Promise((resolve, reject) => {
            Promise.resolve()
                ...
                let duration;
                for (let i = 0; i < 30; i++) {
                    duration = this.isBackground ? '-1' : yield this.getEntry('format=duration');
                ...
                const channels = this.hasAudio ? yield this.getEntry('stream=channels') : '-1';
